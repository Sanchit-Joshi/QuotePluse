...
### PdfService
Loads the full document (with relations) via repository, passes it to a React server-rendered template (`templates/quotation-pdf/`), captures the rendered HTML, and uses Playwright (headless Chromium) to print it to a PDF buffer with fixed page size (A4) and margins matching the template's CSS `@page` rules. Logs every generation attempt (id, durationMs, success) to the structured logger (audit + pdf-generation log categories, see [security.md](security.md)).
